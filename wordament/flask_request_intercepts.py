...
logger = logging.getLogger('wordament.flask.intercepts')
...
class flask_intercepts():
    ...
    def before_request():
        ...
        logger.debug('Request from "{0}" to {2} "{1}"'.format(request.remote_addr, request.full_path, request.method.lower()), extra=log_extra) 
    ...
    def after_request(response):
        ...
        if response.status_code != 500:
            logger.info('{3} response to request from "{0}" to {2} "{1}"'.format(request.remote_addr, request.full_path, request.method.lower(), response.status), extra=log_extra) 
        else:
            log_extra['response_payload'] = response.response
            logger.error('{3} response to request from "{0}" to {2} "{1}"'.format(request.remote_addr, request.full_path, request.method.lower(), response.status), extra=log_extra) 
